...
- Treat repository submission formatting as a hard dependency, not an optional cleanup. Before every `candidate-ready.json`, apply the rules in `skills/format-solution/SKILL.md` to the exact Rainier `solution.md` path, using `workspace/rainier-problem/problemNN-*` as an explicit path override, and apply `skills/_shared/hard_gates.md`. Mirror the current field constants in `scripts/adv` rather than trusting stale limits.
