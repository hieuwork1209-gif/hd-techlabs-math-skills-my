...
$$
We now justify the cyclic invariant ring. Since $p\ge29$, the characteristic does not divide $6$, so both $2$ and $3$ are invertible. Let $\tau$ be the transposition exchanging $a$ and $b$. If a polynomial $h$ is fixed by the cycle $\rho$, then $\tau h$ is also fixed by $\rho$, because $\rho\tau=\tau\rho^{-1}$. Hence
$$
h_+=\frac{h+\tau h}{2},\qquad h_-=\frac{h-\tau h}{2}
$$
are both $\rho$-invariant. The polynomial $h_+$ is fixed by both $\rho$ and $\tau$, which generate $S_3$, so $h_+$ is symmetric. The polynomial $h_-$ is fixed by $\rho$ and changes sign under $\tau$, so it is alternating. Every alternating polynomial is divisible by
$$
\Delta=(a-b)(b-c)(c-a),
$$
and after dividing by $\Delta$ the quotient is symmetric. Thus every cyclic invariant has a unique decomposition
$$
h=F+\Delta G
$$
with $F,G$ symmetric. The decomposition is unique because applying $\tau$ to $F+\Delta G=0$ gives $F-\Delta G=0$, so $2\Delta G=0$; since $\mathbb F_p[a,b,c]/(a+b+c)\cong\mathbb F_p[a,b]$ is a domain and $2\ne0$, this forces $G=0$ and then $F=0$.

By the fundamental description of symmetric polynomials, the symmetric subring is generated by the elementary symmetric functions
$$
e_1=a+b+c,\qquad e_2=ab+bc+ca,
\qquad e_3=abc.
$$
After imposing $e_1=0$, this becomes $\mathbb F_p[s,t]$. Finally,
$$
(u-a)(u-b)(u-c)=u^3+su-t,
$$
and the discriminant of this cubic is both
$$
(a-b)^2(b-c)^2(c-a)^2=\Delta^2
$$
and
$$
-4s^3-27t^2.
...
$$
Consequently the cyclic invariant ring is
$$
E=\mathbb F_p[s,t,\Delta]\Big/(\Delta^2+4s^3+27t^2),
$$
and, as a module over $\mathbb F_p[s,t]$,
$$
E=\mathbb F_p[s,t]\oplus\Delta\mathbb F_p[s,t].
$$
...
$$
Using the module decomposition from Step 1, with
$$
R_0=\mathbb F_p[s,t]/(s^{2q},t^q),
$$
we obtain
$$
Q\cong R_0/(H^{q-1})\oplus
\Delta\,R_0/(H^{q-1}),
$$
so
$$
