...
$$
Each such base orbit gives exactly $Q$ cycles of length $2$, while a nonzero translation has order $\ell$ and gives cycles of length $2\ell$. Thus the full cycle type is determined by $R_w$. The two stated gaps are therefore equivalent to
$$
...
p_L\in\{q,q+2Q,q+4Q\}.
$$
Because $p_L$ is inert, the principal ideal generated by $M(\chi)$ has the same norm as $(p_L)$, hence $(M(\chi))=(p_L)$. Therefore $M(\chi)/p_L$ is an algebraic integer all of whose conjugates have absolute value $1$. By Kronecker's lemma, such an algebraic integer is a root of unity.
...
m_z=E\quad(z\notin U). \tag{8}
$$
The four values are distinct, so the profile recovers the flag. Conversely, for every flag $W\subset U$ of these codimensions, (8) has total mass $m$, first moment $0$, and Fourier magnitudes exactly as in (4). Hence it is realized by block labels and gives precisely the required three cycle types and the two prescribed gaps.
...
$$
Therefore, for every transposition block $T$,
$$
Z_T'=Z_{c^{-1}T}. \tag{9}
$$
So the vector $s=(s_1,\ldots,s_r)$ and the multiplicity profile $(m_z)_{z\in G}$ are invariants of a $\widetilde C_\pi$-conjugacy orbit.

Conversely, suppose two lifts have the same $s$ and the same profile. Since $C_{S_n}(\pi)\cong C_2\wr S_m$ permutes the $m$ transposition blocks arbitrarily, choose $c\in C_{S_n}(\pi)$ that matches blocks label by label. After conjugating by a lift of $c$, the two lifts have the same block sum on every transposition. Their remaining exponent difference on a block $\{i,\pi(i)\}$ is therefore of the form $(\delta,-\delta)$ in each Kummer family. Choose $b_i^{(j)}$ block by block with
$$
b_{\pi(i)}^{(j)}-b_i^{(j)}=\delta.
$$
The additive constant on each block is free, and these constants can be chosen so that $\sum_i b_i^{(j)}=0$ for every $j$; hence they define an element of $\operatorname{Gal}(L/M)$. Conjugating by it removes all remaining endpoint differences. Thus two lifts are $\widetilde C_\pi$-conjugate exactly when they have the same $s$ and the same multiplicity profile.

There are $(\ell-1)^r$ choices for $s$. By Step 3, qualifying profiles are in bijection with flags $W\subset U$ with codimensions $2$ and $1$. The number of hyperplanes $U$ is
$$
\frac{\ell^r-1}{\ell-1},
$$
and for each $U$ the number of hyperplanes $W$ inside the $(r-1)$-dimensional space $U$ is
$$
\frac{\ell^{r-1}-1}{\ell-1}.
$$
Hence the number of orbits is
$$
(\ell-1)^r\frac{\ell^r-1}{\ell-1}\frac{\ell^{r-1}-1}{\ell-1}
=(\ell^r-1)(\ell^{r-1}-1)(\ell-1)^{r-2}.
$$
Final Answer: $\boxed{(\ell^r-1)(\ell^{r-1}-1)(\ell-1)^{r-2}}$
