...
Let $\ell\ge5$ be prime with $2$ generating $\mathbb F_\ell^\times$, let $r\ge3$ be prime with $r\mid\ell-1$, put $Q=\ell^r$, and suppose $q,q+2Q,q+4Q$ are prime with $2q\equiv1\pmod Q$. Put $n=4q^2$. Let $K/\mathbb Q$ be Galois with group $S_n$, let $\zeta$ be a primitive $\ell$th root, assume $K\cap\mathbb Q(\zeta)=\mathbb Q$, and set $M=K(\zeta)$.
...
$$
Writing $\sigma(\rho_j)=\zeta^{s_j}\rho_j$, let $P$ cyclically shift coordinates and, for $0\le t<r$, set $c_s^{(t)}=(s_{1+t}/s_1,\ldots,s_{r+t}/s_r)$. Also require
$$
\mathbf1\in\operatorname{span}\{s,Ps\},\qquad \sum_js_j=r,\qquad \prod_js_j=2,
$$
and $c_s^{(t)}\in U\setminus W$ for every $t$. Give a closed formula.
...
Cyclic Kummer relations couple the block-label first moment to the radical phases. The cycle data determine a hidden Fourier flag, while additive and multiplicative phase constraints force a periodic affine orbit whose projective incidence controls the final centralizer-orbit count, so Field theory remains the appropriate sub-domain.
