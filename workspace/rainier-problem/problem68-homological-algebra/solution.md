...
$$
so the cochain word model consists of an initial $E_T$-block followed by any number of pairs consisting of one of two degree-$2$ socle-separator labels and another $E_T$-block. If $L$ denotes the two-dimensional graded space spanned by those separator labels, concentrated in degree $2$, then as a graded vector space
$$
E\cong\bigoplus_{\ell\ge0}E_T\otimes(L\otimes E_T)^{\otimes\ell}.
$$
Thus a basis word can be written uniquely as
$$
a_0[\sigma_1]a_1\cdots[\sigma_\ell]a_\ell,
\qquad a_i\in E_T,\quad \sigma_i\in L.
$$
The copy of $E_T$ acts on this word model through the leftmost coefficient block. In particular, for $w\in W\subseteq E_T$,
$$
w\cdot\bigl(a_0[\sigma_1]a_1\cdots[\sigma_\ell]a_\ell\bigr)
=(wa_0)[\sigma_1]a_1\cdots[\sigma_\ell]a_\ell.
$$
No later $E_T$-block is changed. Consequently the right ideal generated by $W$ is exactly
$$
WE=\bigoplus_{\ell\ge0}(WE_T)\otimes(L\otimes E_T)^{\otimes\ell},
$$
and therefore
$$
E/WE\cong\bigoplus_{\ell\ge0}(E_T/WE_T)\otimes(L\otimes E_T)^{\otimes\ell}
$$
as graded vector spaces. This is the explicit reason that modding out by $WE$ replaces only the first $E_T$-block by $E_T/WE_T$ and leaves every separator and every later $E_T$-block unchanged. Taking Hilbert series now gives
$$
